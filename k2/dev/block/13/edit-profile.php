<?
$LIB['TOOL']->Delayed('TITLE', 'Редактировать профиль');
$LIB['NAV']->BackAdd(2, array('Редактировать профиль'));
if($_POST){
	if($LIB['USER']->Edit($USER['ID'], $_POST)){
   		$arUser = $LIB['USER']->ID($USER['ID']);
		Redirect('?action=edit-profile&complite=1');
	}else{
...
?><form action="<?=$CURRENT['PATH']?>?action=edit-profile" method="post" enctype="multipart/form-data" id="form">
	<table class="table">
		<tr>
			<td width="100"></td>
			<td><?=$sMessage?></td>
