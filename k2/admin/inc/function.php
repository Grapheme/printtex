...
{
	?><br><table id="warning" style="margin-left:16px;"><tr><td>У вас нет прав доступа к этой странице</td></tr></table><?
...
function permissionCheck($sType, $sKey = 'INDEX')
{
	...
	if(($sType == 'SECTION_PERMISSION') && ($USER['USER_GROUP'] != 1)){
		permissionDenied();
	}else
	if(($sType == 'MODULE') && ($sKey == 'INDEX')){
		if(!$USER['PERMISSION']['DEFAULT']['MODULE']){
			permissionDenied();
		}
	}else
	if(!$USER['PERMISSION']['DEFAULT'][$sType][$sKey]){
		permissionDenied();
	}
}
...
function tableBody($arPar)
{
	...
	foreach($arPar['CONTENT'] as $sKey => $sText)
	{		$arField = $arPar['FIELD'][$sKey];
		...
		?>><?
		if($arField['FORMAT'] == 'DATE'){
			plus(dateFormat($sText));
		}else
		if($arField['FORMAT'] == 'USER'){
			if($arPar['USER_LOGIN'][$arValue[0]]){
				?><a href="/k2/admin/user/edit.php?id=<?=$arValue[0]?>"><?=html($arPar['USER_LOGIN'][$arValue[0]])?></a><?
				if($arValue[1]){					?> ...<?
				}
			}else{				?>-<?
			}
		}else
		if($arField['FORMAT'] == 'USER_GROUP'){
			if($arPar['CONTENT']['_USER_GROUP_NAME']){
				?><a href="/k2/admin/user/group/edit.php?id=<?=$sText?>"><?=html($arPar['CONTENT']['_USER_GROUP_NAME'])?></a><?
			}else{
				?>-<?
			}
		}else
		if($arField['FORMAT'] == 'SELECT'){
		...
		if($arField['FORMAT'] == 'SECTION'){
			if($arPar['CONTENT'][$sKey]){
				?><a href="/k2/admin/section/content/?section=<?=$arValue[0]?>"><?=html($arPar['CONTENT']['_'.$sKey.'_NAME'])?></a><?
				if($arValue[1]){
					?> ...<?
				}
			}else{
		...
		if($arField['FORMAT'] == 'FILE'){
			if($arPar['CONTENT'][$sKey]){
	            if($arPar['PREVIEW'] && $arPar['CONTENT']['_'.$sKey.'_WIDTH']){
					if($arPhoto = $LIB['PHOTO']->Preview($arValue[0], array('WIDTH' => 60, 'HEIGHT' => 60))){
		                ?><a href="/files/<?=$arPar['CONTENT']['_'.$sKey.'_PATH']?>" title="<?=$arPar['CONTENT']['_'.$sKey.'_NAME']?> (<?=$arPar['CONTENT']['_'.$sKey.'_WIDTH']?>x<?=$arPar['CONTENT']['_'.$sKey.'_HEIGHT']?> <?=fileByte($arPar['CONTENT']['_'.$sKey.'_SIZE'])?>)" target="_blank"><img src="<?=$arPhoto['PATH']?>" width="<?=$arPhoto['WIDTH']?>" height="<?=$arPhoto['HEIGHT']?>" class="preview"></a><?
					}
				}else{
					?><a href="/files/<?=$arPar['CONTENT']['_'.$sKey.'_PATH']?>" title="<?
					if($arPar['CONTENT']['_'.$sKey.'_WIDTH']){
						echo $arPar['CONTENT']['_'.$sKey.'_WIDTH'].'x'.$arPar['CONTENT']['_'.$sKey.'_HEIGHT'].' ';
					}
					?><?=fileByte($arPar['CONTENT']['_'.$sKey.'_SIZE'])?>" target="_blank"><?=$arPar['CONTENT']['_'.$sKey.'_NAME']?></a><?
					if($arValue[1]){
						?> ...<?
					}
				}
		...
		if($arField['FORMAT'] == 'CATEGORY_NAME'){
        	?><a href="<?=$arPar['SECTION']['URL']?>" class="sectionLink"><span class="icon section"></span><?=html($sText)?></a><?
		}else
		if($arField['FORMAT'] == 'TORF'){
			echo ($sText?'Да':'Нет');
		}else{
			$sText = strip_tags(str_replace(array("\r", "\n"), array('', ''), $sText));
           	if(strlen($sText)>200){
				$sText = preg_replace("#(.+)\s.+?$#", "\\1", substr($sText, 0, 200)).' ...';
           	}
			echo html($sText);
		}
		?></td><?
...
function fieldFormat($sTable, $arField)
{	global $LIB;
	...
	{
		$sFormat = $arFieldFormat[$arFields[$i]['TYPE']];
		if($arFields[$i]['TYPE'] == 5){
	    	$arData = unserialize($arFields[$i]['SETTING']);
	    	if($arData['TYPE'] == 0){
	    		$sFormat = 'SECTION';
	    	}
	    	if($arData['TYPE'] == 1){
