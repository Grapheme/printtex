...
if($_POST){
	...
	if($nID = $LIB['COMPONENT_ELEMENT']->Add($_COMPONENT, $_COLLECTION, $_POST)){
		if($_POST['BAPPLY_x']){
...
?><div class="content">
    ...
    <form method="post" enctype="multipart/form-data" class="form">
		...
		if(!isset($_POST['SORT'])){
	    	$_POST['SORT'] = 10;
	    	if($arContent = $DB->Row("SELECT `SORT` FROM `k2_component".$_COMPONENT."` WHERE `COLLECTION` = '".$_COLLECTION."' ORDER BY `ID` DESC LIMIT 1")){
		...
		<div class="saveBlock">
			...
			<p><input type="submit" class="sub" value="Сохранить"> или <a href="/k2/admin/system/component/<?=$sURL?>">отменить</a></p>
		</div>
    </form>
</div><?
