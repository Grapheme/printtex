...
$arRows = $DB->Rows("SHOW COLUMNS FROM `k2_component".$_COMPONENT."category`");
$arField = fieldFormat('k2_component'.$_COMPONENT.'category', $arField);

$QB = new QueryBuilder;
$QB->From('k2_component'.$_COMPONENT.'category AS B');
...
$QB->OrderBy($arSort['FIELD'].' '.$arSort['METHOD']);
...
?><table width="100%" class="nav">
	<tr>
		<td><?=$sNav?></td>
		<td align="right"><a href="add.php<?=$sURI?>" class="button">Добавить категорию</a><?
		if(!$arCount['FOUND_ROWS()']){
			?><a href="../element/add.php<?=$sURI?>" class="button">Добавить элемент</a><?
		}
		?></td>
		</tr>
</table>
	...
	<table width="100%" class="table">
		...
		for($i=0; $i<count($arList); $i++)
		{
			...
			?>" goto="edit.php<?=$sURL?>&id=<?=$arList[$i]['ID']?>">
				...
				'SECTION' => array('URL' => '/k2/admin/system/component/?field='.$_FIELD.'&component='.$_COMPONENT.'&collection='.$_COLLECTION.'&category='.$arList[$i]['ID'])
				));
				?>
		...
		?><table width="100%" class="select">
	    	<tr>
	            ...
	            $('#action').change(function(){
	                ...
	                if(data.length){
	                	if(val == 'delete'){
	                		$.prompt(this, {'href':'/k2/admin/system/component/category/delete.php<?=$sURI?>', 'yes':'return actionDelete(1)', 'no':'return actionDelete(0)'});
	                	}
	                }
	            });
	            ...
	            <td align="right">На странице <select id="sizePage" url="/k2/admin/system/component/category/<?=$sURI?>&"><?
