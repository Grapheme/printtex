...
if($_POST){
	...
	if($_POST['URL']){
		if(preg_match("#.+/(.*?)\.(jpg|jpeg|gif|png|bmp)$#i", $_POST['URL'], $arMath) && ($sCont = urlContent($_POST['URL']))){
			$arPar = array(
			...
			<td class="fm-table-l">
				<div class="fm-body">
					<table class="table" width="100%">
						...
						{
	                    	?><tr align="center">
								<td align="left"><a href="/files/<?=$arFile[$i]['PATH']?>" onclick="return fm.click('?field=<?=$_REQUEST['field']?>&parent=<?=$arFile[$i]['DIR']?>&file=<?=$arFile[$i]['ID']?>')" ondblclick="return fm.dblclick('<?=$_REQUEST['field']?>', {'path':'<?=$arFile[$i]['PATH']?>', 'width':'<?=$arFile[$i]['WIDTH']?>', 'height':'<?=$arFile[$i]['HEIGHT']?>'})" class="fmIcon"><img src="/k2/admin/i/ext/<?=fileIcon($arFile[$i]['PATH'])?>"><?=html($arFile[$i]['NAME'])?></a></td>
