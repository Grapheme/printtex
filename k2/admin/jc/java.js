...
function setComponent(field)
{
    ...
    $.layer({'get':'/k2/admin/system/component/component.php?field='+field+'&component='+$('#setComponent').val(), 'title':'Компонент &laquo;'+$('#setComponent option:selected').text()+'&raquo;', w:800, 'resize':1});
}
...
var k2 = {
	...
	template:{
		add:function(){
			k2.transcription.init({'igonere_cookie':true, 'suffix':'.php'});
	...
	component:{		icon:function(){
		...
		past:function(par)
		{
			...
			$.post('/k2/admin/system/component/field.php', {'component':par.component, 'field':par.field, 'collection':par.collection});
			$('#layer').remove();
		}
	},
	tool:{		layer:function(id){
	...
	layer:function(id, _this){
 	...
 	dblclick:function(field, obj){
		...
		if(tinyMCE.get(field).selection.getContent()){
	    	tinyMCE.get(field).execCommand('mceReplaceContent', false, '<a href="/files/'+obj.path+'">{$selection}</a>');
	    	$('#layer', window.parent.document).remove();
		}else if(obj.width>0){
	 		tinyMCE.get(field).execCommand('mceInsertContent', false, '<img src="/files/'+obj.path+'" width="'+obj.width+'" height="'+obj.height+'">');
	 		$('#layer', window.parent.document).remove();
		}
		return false;
 	}
...
var table = {
	...
	check:{		all:function(_this, tbl){			$('input[type=checkbox]', tbl)[$(_this).attr('checked')?'attr':'removeAttr']('checked', 'checked');
		}
	}
}
...
var tree = {
	...
	stop:function(){
		...
		$('#tree a').removeClass('tree-move-a-hover').die('mouseover mouseout');
		$('.tree-move-border').removeClass('tree-move-border-hover').die('mouseover mouseout');
		tree.first = false;
        tree.complite();
		tree.elm_to = false;
		tree.code = 0;
	...
	complite:function()
	{
        ...
        $.prompt(this, {'href':'/k2/admin/section/move.php?section_from='+$(tree.elm).attr('section')+'&section_to='+fromTo+'&code='+tree.code, 'text':'Перенести выбранный раздел?'});
	}
}
...
$(function(){
	...
	$('.filetree a').mousedown(function(e){
		if(e.button == 2){
            ...
            '<div class="line"></div>'+
            '<a href="'+url+'" blank="true">Посмотреть на сайте</a>'+
            '</div>';
	...
	$('.fileMultiple').MultiFile({		STRING:{			remove:'<span class="closeMini" title="Удалить"></span>',
