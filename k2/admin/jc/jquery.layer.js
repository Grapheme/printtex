(function($){
	$.layer = function(par){
		...
		$.post(par.get, query, function(data){
            ...
            layer.css({'left':Math.ceil(($('body').width()/2)-(layer.width()/2) + scrolLeft), 'top':Math.ceil(($('body').height()/2)-(layer.height()/2) + scrollTop)});
            layer.show();
	...
	$.prompt = function(_this){
        par = {title:'Системное сообщение', 'text':'Действительно удалить?', 'href':_this.href, 'yes':'', 'no':'$(\'#layer\').remove();'};
        if(typeof(arguments[1]) != 'undefined'){
