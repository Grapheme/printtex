...
tab_(array(array('Настройки', '/dev/component/edit.php?id='.$_ID), array('Поля категорий', '/dev/component/field/category/?id='.$_ID, 1), array('Поля элементов', '/dev/component/field/element/?id='.$_ID)));
if(!$arComponent = $LIB['COMPONENT']->ID($_ID)){
	Redirect('/k2/admin/dev/component/');
}
if($_POST){
	if($nID = $LIB['FIELD']->Add('k2_component'.$_ID.'category', $_POST)){
		if($_POST['BAPPLY_x']){
			Redirect('edit.php?id='.$nID.'&complite=1');
		}else{
			Redirect('/k2/admin/dev/component/field/category/?id='.$_ID);
		}
	}
}
?><div class="content">
    ...
    <form action="add.php?id=<?=$_ID?>&amp;TYPE=<?=(int)$_REQUEST['TYPE']?>" method="post" class="form">
		...
		<div class="saveBlock">
			...
			<p><input type="submit" class="sub" value="Сохранить"> или <a href="/k2/admin/dev/component/field/category/?id=<?=$_ID?>">отменить</a></p>
		</div>
    </form>
