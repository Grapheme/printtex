...
tab_(array(array('Настройки', '/dev/block/edit.php?id='.$nBlock), array('Поля категорий', '/dev/block/field/category/?id='.$nBlock), array('Поля элементов', '/dev/block/field/element/?id='.$nBlock, 1)));

if($_POST){
	if($LIB['FIELD']->Edit($_ID, $_POST)){
		if($_POST['BAPPLY_x']){
...
?><div class="content">
	...
	<form action="edit.php?id=<?=$_ID?>" method="post" class="form">
		...
		?><div class="saveBlock">
			...
			<p><input type="submit" class="sub" value="Сохранить"> или <a href="/k2/admin/dev/block/field/element/?id=<?=$nBlock?>">отменить</a></p>
		</div>
	</form>
</div><?
