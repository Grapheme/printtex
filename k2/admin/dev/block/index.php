...
<div class="content">
    ...
    <table width="100%" class="nav">
    	<tr>
            <td align="right"><?
            if($arGroup){
				?><a href="import/" class="button">Импортировать</a><a href="#" onclick="return $.layer({get:'/k2/admin/dev/group/add.php', title:'Добавить группу', w:397}, function(){k2.group.add()})" class="button">Добавить группу</a><a href="add.php" class="button">Добавить блок</a><?
			}else{
				?><a href="#" onclick="return $.layer({get:'/k2/admin/dev/group/add.php', title:'Добавить группу', w:397}, function(){k2.group.add()})" class="button">Добавить группу</a><?
			}
            ?></td>
        </tr>
    </table>
		...
		for($i=0; $i<count($arGroup); $i++)
		{
			...
			for($n=0; $n<count($arBlock); $n++)
			{
				...
				?>" goto="edit.php?id=<?=$arBlock[$n]['ID']?>">
					...
					<td align="center"><a href="delete.php?id=<?=$arBlock[$n]['ID']?>" onclick="return $.prompt(this)" class="icon delete" title="Удалить"></a><a href="edit.php?id=<?=$arBlock[$n]['ID']?>" class="icon edit" title="Редактировать"></a></td>
				</tr><?
			}
		}
    	if(!$i){
