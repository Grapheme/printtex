...
$arElement = $LIB['BLOCK_ELEMENT']->ID($_ID, $_SECTION_BLOCK);

if(!$arSBlock || !$arSection || !$arBlock || !$arElement){
	Redirect('/k2/admin/');
}
...
{
	$arTab[] = array($arSBlockList[$i]['NAME'], '/section/content/?section='.$_SECTION.'&block='.$arSBlockList[$i]['ID'], ($_SECTION_BLOCK == $arSBlockList[$i]['ID']));
}
if($arTab){
	tab_($arTab);
}
...
if($_POST){
	if($LIB['BLOCK_ELEMENT']->Edit($_ID, $_SECTION_BLOCK, $_POST)){
		if($_POST['BAPPLY_x']){
			Redirect('edit.php?section='.$_SECTION.'&section_block='.$_SECTION_BLOCK.'&id='.$_ID.'&category='.$_CATEGORY.'&complite=1');
		}else{
...
?><div class="content">
    ...
    <form action="edit.php?section=<?=$_SECTION?>&section_block=<?=$_SECTION_BLOCK?>&id=<?=$_ID?>&category=<?=$_CATEGORY?>" method="post" enctype="multipart/form-data" class="form">
